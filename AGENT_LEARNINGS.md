...
- **Mistake:** merged #41, #42 and #43 while `main` CI had been red for 5 runs (11 VPS-environment tests: pm-shim `node`, `/root/claude` paths, host `dist/`) and only noticed on #43 → before any merge run `gh run list --branch main --limit 3`; a red main is an open defect to log, and diff the PR's failing set against main's instead of assuming.
