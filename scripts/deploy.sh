...
# (user_version=6, baseline_history dropped). A broken/next migration fails the
...
if [[ "${MC_SKIP_MIGRATION_GATE:-0}" != "1" ]]; then
  ...
  if npx tsx scripts/validate-migration-runner.ts fresh "$MIG_TMP/fresh.db" > "$MIG_TMP/out.json" 2> "$MIG_TMP/err.log"; then
    ...
    if [[ -n "$MIG_JSON" ]] \
      && grep -qE '"userVersion": *6' <<<"$MIG_JSON" \
      && ! grep -q '"baseline_history":' <<<"$MIG_JSON"; then
      echo "[deploy] Migration gate OK — fresh boot → user_version=6, baseline_history absent."
    else
      echo "[deploy] FAILED — fresh-boot schema signature unexpected (want user_version=6, no baseline_history table):"
