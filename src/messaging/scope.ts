...
export const CORE_TOOLS = [
  ...
  "data_summarize", // Always available — deterministic counts/stats over CSV/JSONL/markdown (usability Phase 3.1)
...
// Static previews (/root/claude/previews/<name> → <name>.187.77.25.101.nip.io,
// vhost generated by preview-caddy-sync) are directories on the real filesystem:
// publishing AND closing one needs file_write / file_delete / shell_exec.
// 2026-09-19: "Cierra el Caddy" and "Cierra el tema Bimaso…" classified as
// specialty / destructive+projects — no file tool in scope, both tasks ended
// `blocked` ("Necesito shell_exec", tasks 0a376e6d / 66fc5038). `destructive`
// carries no tools of its own. UNAMBIGUOUS tokens only: this grants the heavy
...
export const DEFAULT_SCOPE_PATTERNS: ScopePattern[] = [
  ...
  {
    // Publish/close a static preview (Caddy / nip.io) — shared PREVIEW_SITE_RE.
    pattern: PREVIEW_SITE_RE,
    group: "coding",
  },
  {
  ...
  {
    pattern:
      /\b((?:publica(?:r|ci[oó]n)?|publique)\s+(?:\S+\s+){0,3}(?:en\s+)?(?:redes|instagram|facebook|tiktok|youtube|x|twitter)|publish\s+(?:to|on)\s|postea(?:r|lo)?|tweet|re-?tweet|tuit(?:ea|ear|[eé]alo|[eé]ame)?|twitter|x\.com|tweet_post|tweet_probe|tweet_mentions|(?:menci[oó]n(?:es)?|mention(?:s|ed)?)\s+(?:de\s+|en\s+|on\s+|in\s+)?(?:x\b|twitter)|(?:x\b|twitter)\s+mentions?|(?:sesi[oó]n|cuenta|acceso|credencial(?:es)?|cookies?|login|token)\s+(?:\S+\s+){0,4}(?:de|en|a|para|to|of|del)\s+(?:twitter\b|x\b(?!\s+\w))|instagram|facebook(?!\s*auth)|redes\s*sociales|social\s*media|contenido\s+para\s+(?:redes|social)|programar?\s+(?:un\s+)?post|schedule\s+post)\b/i,
    group: "social",
  },
  {
    pattern:
