...
```
Seis filas o más: las skills se usaron (`skill_load` cuenta: el modelo carga el cuerpo de la skill y lo aplica en contexto; `skill_run` la ejecuta como llamada aparte — ambas valen). Cero filas: el brazo A fue en realidad un brazo B y la evaluación no vale. Resultado del brazo A (2026-09-13 01:40–02:05 UTC, PID 120262): los seis turnos se clasificaron `skills, coding` por el clasificador semántico. Uso de skills por brief: S1 `skill_load` short-form-script · S2 **solo documentos** (3 lecturas del KB, sin skill) · M1 `skill_load` · M2 `skill_load` · B1 **solo documentos** (3 lecturas, sin skill) · B2 `skill_load` ×2 + `skill_run` ×2 (logline-premise-test y series-engine-test). Los seis usaron el corpus; cuatro usaron además el cuerpo de la skill. Anótalo en la hoja: S2 y B1 miden el corpus sin la skill. Primera prueba en vivo 01:41 UTC (S1-A, tarea `a11489a9`): clasificador semántico → `skills, coding`; llamadas `ToolSearch → jarvis_file_read (00-index) → skill_load (short-form-script) → jarvis_file_read (doc 13) → file_write`; `short-form-script.use_count` 7 → 8.
