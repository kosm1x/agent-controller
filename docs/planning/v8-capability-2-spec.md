...
-- 6. ACCEPTANCE — REMOVED FROM THE GATE 2026-08-02 (see the note under this block)
--    (a) green/red promote ratio ≥ 1.5×  (b) ≥10 consecutive operator-accepted judgments, 0 "Audited?" cycles
-- NOTE: the SQL below is the ORIGINAL judgment-grain form. Kept ONLY for the
--       collapse it illustrates. Neither it nor the brief-grain rewrite that
--       replaced it (2026-06-26) is in the gate any more — §17 is five checks.
SELECT confidence,
...
**6a REMOVED (2026-08-02) — it asked an unanswerable question.** History: the judgment-grain SQL above COLLAPSES on real briefs (promotion is a per-BRIEF event, so a promoted mixed-color brief counts every judgment — green AND red — as promoted → green-rate ≈ red-rate → ratio → 1.0). It was recalibrated to brief-grain on 2026-06-26 (each brief scored by its **highest-leverage** judgment's color, else plurality, ties → more cautious), then rebased on explicit verdicts on 2026-07-10. Both rebuilds landed at ~1.0 anyway, and on 2026-08-02 the diagnosis came back structural rather than fixable:

1. **`confidence` is not a usefulness estimate.** §12 computes it MECHANICALLY from evidence density (`lib/v8-2/confidence.ts`): green = ≥3 distinct sources, no contradictions, nothing stale; red = thin/contradicted/stale. `red` means "thin evidence", not "probably wrong" — and a thinly-sourced item can be the most valuable thing in the brief. 6a scored the operator for declining to reject those.
2. **The scored population never existed.** Of the 20 operator-ruled briefs in the 2026-07-10 → 08-02 window: 8 pure-green, 12 mixed, **0 pure-red or pure-yellow**. Every red judgment shipped alongside green ones, so "a red brief" was a label `briefConfidenceColor` manufactured from a mixed set. Moving the number required discarding briefs that were good.
3. **One verdict, many colors.** `sirve`/`descarta` is per-BRIEF; confidence is per-JUDGMENT. No grain bridges that — which is why both rebuilds were pinned near 1.0 by arithmetic rather than by miscalibration (judgment-grain 71.8% vs 75.0%; brief-grain 68.8% vs 66.7%).

No substitute signal exists either: `judgments.concession_kind` was populated on 1 of 58 rows, and critic approval is flat by color (green 90% / red 89% / yellow 88%) because the §11 critic loop gates every judgment BEFORE delivery. **Nothing replaced it.** "Are the briefs useful?" is measured by the V8.1 §13 morning promote-rate (≥60%, 75% at removal); per-verdict data stays visible via `mc-ctl verdict-rate`. Proving confidence CALIBRATION needs a **per-judgment verdict affordance** that does not exist — build that before re-adding any ratio over `confidence`. Operator ruling 2026-08-02.
...
**Operational metrics (post-activation):** strategic-thinking-fraction (Licklider 85/15: `decide|formulate|judge` vs `retrieve|search|summarize`; baseline ≈15%, target ≥30%/90d); ~~color-promote correlation ≥1.5×/30d~~ (dropped with 6a, 2026-08-02 — unmeasurable without a per-judgment verdict); resolver ≥95%; unfixable ≤5%; sycophancy ≤5%; p50 ≤30s / p95 ≤60s; cost ≤$0.15/brief (shadow-confirmed, §18 Q1).

**Watchpoints:** sycophancy >5% (voice or cache-prefix issue); resolver <90% (ledger gaps / unsatisfiable boundaries); unfixable >10% (overconfident author or thin evidence); **diversity-gate retry >30%** (Synthesizer collapsing — likely; the gate is advisory, so this triggers review, not auto-fail); p95 >90s (parallelize critic+confidence).
...
3. Day 1–7: `mc-ctl briefing-gate` returns `insufficient_data` for §17 while volume accrues. NOTE (2026-08-02): the combined verdict is now TRUE worst-of-two — an `insufficient_data` on EITHER gate makes the combined verdict `insufficient_data` (exit 2). This step previously read "V8.1 §13 stays exit 0 — the combined verdict does NOT demote a passing V8.1 during the V8.2 shadow"; that `||`-pass exception existed only because 6a pinned §17 non-green, and it was removed with 6a. Watch the §17 section's five checks + the watchpoints above.
4. **Delivery (Phase 10, SHIPPED 2026-06-26 — dormant behind `V82_DELIVERY_ENABLED`):** set `V82_DELIVERY_ENABLED=true` → restart; the 06:00 brief now appends the strategic-judgment section. Requires `V81_BRIEF_DELIVERY_ENABLED=true` (the V8.1 brief must itself be delivered) and `V82_JUDGMENT_PRODUCER_ENABLED=true` (so judgments exist). NOTE: delivery is **no longer a §17 prerequisite** — 6a was the only check that needed an operator verdict, and it was removed 2026-08-02, so §17 can now go green on shadow data alone. Arming delivery is a product decision (do the judgments belong in the operator's brief?), not a gate unblock.
