...
4. Day ≥7 + ≥10 judgments: when §17 reads `pass` AND the operator + Jarvis agree the principle block is the desired voice (bilateral-maturity), V8.2 is activatable. Surfacing V8.2 judgments into the delivered brief (the "delivery" layer) + brief-grain acceptance recalibration (§17 6a note) are the post-shadow build.
5. Kill switch: `V82_JUDGMENT_PRODUCER_ENABLED=false` → restart (rows stop, cron unregisters on next boot, concession path reverts to V8.1 regex).
