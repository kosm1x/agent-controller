...
**Residuals + context-05 status (2026-09-22):** shipped `ed4eb07`; DEPLOYED 2026-09-22 23:40 UTC PID 1314208 (was 1032972): `/health` 200, 0 startup errors, INDEX.md regenerated at boot (`updated_at` 23:40:50, no `[[/|]]`), dist carries `realResolve(abs)` + executor `cacheable: false`, `jarvis-fs.js` no longer calls `regenerateIndex`. context-05 DONE: `buildGoalPrompt` returns a stable part (header, always-read + enforce KB, instructions) and a `cacheable:false` part (task KB, goal, criteria, context, learnings); `compactL3` and the OpenAI wrap-up keep every leading system message; `eval:gate --run` PASS 68.39 vs 68.35 (+0.04, $5.64). Residuals DONE: eval-gate and validate-tool-search score a `VACUUM INTO` snapshot taken over a read-only connection; INDEX.md is regenerated at service boot, not in `initDatabase()`; the `[[/|]]` entry is gone (the `a`/`p` rows need the operator's `mc-ctl db` cleanup); the pgvector timer is cleared. The six A+B residuals DONE: symlink gating now walks the path like the kernel (`realResolve`/`realResolveParent`: a dangling link counts, `..` applies after a directory link) in validatePathSafety, file_delete, file_edit, the four output-path validators and the google_drive download; glob/list_dir apply the read denylist per entry; secret-name redaction scans names without consuming values (linear); ECharts formatters carrying markup are dropped; `/dashboard/:id` only matches hex ids. qa R1 FAIL → R2 FAIL → R3 FAIL → R4 PASS; 33 mutants RED; 9,257 tests. Out of scope (reported, not fixed): the jarvis-fs KB mirror write and wordpress `/tmp/wp_content/` read gate still check the text path; redaction misses `1API_KEY=` and quoted values with spaces.
