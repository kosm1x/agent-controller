...
| **No versioning / stale skills** | Skills HAVE versions + critic gate + drift check (`skills/lifecycle.ts`), and rollback EXISTS (`mc-ctl skills revert <name> <version>` — this plan first said none; corrected 2026-09-16). Promotion is proposal-only (`skill-discovery.ts:7`). Two skill stores (table vs KB `skills/`, 10 SKILL.md mirroring table rows). |
...
- 115 skills, 13,299 uses / 13,142 successes (30 d cumulative counters), 21 `skill_versions`, `is_certified` = 4 live (`dialogue-on-the-nose-pass`, `logline-premise-test`, `scene-value-turn-diagnostic`, `short-form-script`; the 09-12c fifth skill `series-engine-test` is `is_certified=0` — reconciled 2026-09-16, the seed reported critic PASS, not certification), `consecutive_failures` gate, cosine retrieval on `description_embedding` (`skills/retrieval.ts:66-120`) inside `enrichContext`, critic gate + `body_sha256` drift (`lifecycle.ts:106-240`), evolution ritual 23:00 deactivates < 30 % success at ≥ 5 uses.
- **Gap G7**: promotion is proposal-only (`skill-discovery.ts:24-110`, 3+ occurrences, 24 h rate limit, "does NOT auto-save"); no producer. ~~Gap G8~~ (withdrawn 2026-09-16: `mc-ctl skills revert` already exposes `pointSkillAtVersion`). **Gap G9**: KB `skills/` folder vs `skills` table are two stores (09-09 finding). Activation gate ≥ 5 certified (`mc-ctl:1126-1139`) unmet or barely met.
...
### Phase 0 — Instrument + test scaffold (½ session) — SHIPPED 2026-09-16
Goal: make the checklist and the memory tax observable before touching behavior.

**As shipped**: `mc-ctl memory-checklist [--stale] [--json]` (live readout 4 green · 3 amber · 3 red); `mc_memory_injection_tokens{block=user_facts|enrichment|kb|kb_stable|kb_variable|jme}` (`kb` = the singular builder on the fast-runner read-only branch; no `skills` block — skills text rides inside `enrichment`) + `mc_prompt_section_dropped_total{tier=p2|p3|p4}`; `src/memory/playbook-battery.test.ts` runs amnesia / isolation / staleness against the real store code (contradiction, promotion, load, cron, continuity are `it.todo` naming their phase). `scripts/validate-memory-tests.ts` (live probe) NOT built — the checklist's live rows cover it for now. New finding: `pruneExpiredFacts()` has no caller — 106 expired `jme_facts` sit in the table, filtered at read only (→ P5).
1. `mc-ctl memory-checklist` (`scripts/memory-checklist.ts`, read-only): the paper's 10 checklist items evaluated from live data — episodic log rows/24 h, precedent retrieval calls (0 until P1), semantic stores + row counts, ontology present (no), conflicts flagged (n/a), skills promoted last 30 d, rollback available (yes — `skills revert`), expiry crons last-success timestamps (`mc_ritual_last_success_timestamp`), compaction handoff (no), forgetting schedule. Same shape as `mc-ctl briefing-gate`.
...
### Phase 1 — Episodic precedent retrieval (1 session) — SHADOW SHIPPED 2026-09-16, inject pending readout
Goal: "Agent skips known dead ends" (paper Day 2 exit test), replacing the 0.7 %-utility operational-bank slot.
1. `src/memory/precedents.ts`: `findPrecedents(messageText, {k=3})` over root `Chat:` tasks (90 d, terminal status) ⋈ `task_gates` ⋈ `tasks.output|error` snippet. **As shipped: keyword overlap on the TITLE only** — on live rows `tasks.description` is the assembled system prompt (~24 KB) and `input` is empty, so the title (first ~60 chars of the request) is the only request text; one short-column scan of the window's root `Chat:` rows (~2,100), scored in JS with accent folding on both sides, supersession computed from the same candidate set, gates + snippet fetched for the k winners only. ~55 ms median per call measured live 2026-09-16 (unindexed `tasks` scan; an index is a schema change, deferred to the readout; no embeddings — `tasks` has no embedding column and per-candidate embedding is a network call each). Embedding similarity is a readout decision, not a default. Each hit carries `outcome`, gate verdict (`PASSED|FAILED|ABANDONED`), `supersededBy` (later `completed` task with identical normalized title — the queued 09-09 item), age, and the run's error/resolution snippet.
2. **Deferred to after the readout**: inject as a ≤ 600-token "Precedentes" block in the enrichment slot currently fed by `mc-operational` free-text recall. Shipped instead: `MEMORY_PRECEDENTS_MODE=shadow` (default) computes + logs, injects nothing; `off` disables; any other value (incl. a premature `inject`) folds to `shadow`. The hook in `enrichContext` runs via `setImmediate` outside `recallPromises` — nothing awaits it, but better-sqlite3 is synchronous, so the ~55 ms still lands on the event loop once per chat turn (avg turn 77 s). Sort superseded/failed last, label them; never drop them silently (the 09-09 lesson: unlabeled retrieval launders; dropped retrieval hides the caveat).
...
**Readout caveats (qa audit 2026-09-16, W-4/W-5)**: (a) the utility matcher claims rows by TIME inside a 60 s window, and 31.5 % of chat turns run longer than 60 s — those turns' shadow rows stay unmarked, so read the rate over MARKED rows and stratify `--stratify-by=match_type` next to `--stratify-by=bank`; (b) shadow-row content is the past task's output/error snippet (title only as fallback), so a `was_used` hit means the reply re-derived the OUTCOME, not that it echoed the user's wording — still hand-sample 20 marked rows before reading the number as relevance; (c) ~1.8 % of candidates have no body, fall back to the title, and are marked unused on the first sweep (structural zero). Single-token requests log nothing (no discriminating signal). (d) **First live row (2026-09-16 07:54 UTC, deployed PID 2901375)**: a day-log summary request matched 3 earlier summary tasks and was marked `was_used=1` on the first sweep — recurring daily summaries overlap their predecessors by STRUCTURE, so the marked rate over-reads from day one; (b) is mandatory. Same turn's memory tax in tokens: kb_stable 13.7k ≫ user_facts 3.3k > kb_variable 1.9k > enrichment 1.1k > jme 0.3k — the stable KB layer is where the §F "does retrieval pay for itself" answer will come from. Shadow rows are excluded from the unstratified `audit-claim utility` headline and from the `mc-ctl recall-utility` totals (shown as a separate "Shadow" line).
...
Goal: promotion test; one skill store (rollback already exists).
...
2. ~~`mc-ctl skills rollback`~~ — already exists as `mc-ctl skills revert <name> <version>` (found 2026-09-16); `skill_revise` stays library-only.
...
- ~~Live `is_certified` count and which 4/5 rows.~~ Done 2026-09-16 (4; `series-engine-test` uncertified).
- ~~`task_outcomes` column set~~ — Phase 1 shipped on `tasks.output|error` + `task_gates`; `task_outcomes` unused so far.
