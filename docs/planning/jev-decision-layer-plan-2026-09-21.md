...
### Phase B — offline replay of the classifier question (≈ $0.02; needs the 3 rulings)
...
- Corpus: the most recent 400 `scope_telemetry` rows with a non-empty `tools_called` (3,169 of 3,690 rows qualify on 09-21). Population count printed first; < 200 usable ⇒ stop.
- Request shape per §3.4. Ground truth = the groups that own the tools the turn actually called. Known bias: a tool can only be called if it was in scope, so the live classifier scores ~100 % here by construction — the bar is absolute, and live `active_groups` is used only for the bloat comparison.
- Report split es / en: coverage, mean extra groups, latency p50/p95/max from this host, errors/429s, and the fixed-vs-broke count against the live groups (the cookbook's 37/7 is the shape to expect).
- **PASS** (Spanish turns): coverage ≥ 95 % · mean extra groups ≤ live + 1 · p95 ≤ 800 ms. A FAIL ends the Jev work; record the numbers.
