...
> **2026-04-22 — FEATURE FREEZE (through 2026-05-22 minimum)**: Session 100 closes active feature development. All new functionality is paused for a hardening / credentials / stabilization window. See `docs/planning/stabilization/30d-hardening-plan.md` for the backlog and `docs/planning/stabilization/full-system-audit.md` for the 5-dimension audit methodology. Baseline: `docs/benchmarks/2026-04-22-baseline.md`.
>
...
## v7.7 Substrate Completion Phase — **OPEN** (opened 2026-05-18)

> Guide: `docs/V7.7-GUIDE.md` (operational). Pre-plan: `docs/V7.7-PREPLAN.md` (decision record, READ-ONLY). 7 spines: S2 (self-audit) / S3 (drift detector) / S5 (skills-as-stored-procedures, all 5 phases per Q1) / Conway Patterns 1-3 (all three per Q2) / closure ritual + continuity tooling (per Q4 absorption). Operator-profile skeleton (Q3 carve-out from anti-mission) ships as part of Spine 5 — schema-only, no inference. Anti-mission: no V8.1/V8.2/V8.3 capability work. Operator framing: "v7.7 is pre-8.0 foundation work." ETA close 2026-06-30 → 2026-07-05 (revised from original 2026-07-20 baseline reflecting v7.6's 3× compression dynamics).

| Spine                                                    | Status      | Closure commits | Audit log | Notes                                                                                                                                                                           |
| -------------------------------------------------------- | ----------- | --------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 — S2 self-audit                                        | **Pending** | —               | —         | Recommended entry point; primitive (`mc-ctl audit-claim`) already live (`095647b`); Phase 1 harness + Phase 2 retrofit `morning_brief` + community-manager email + closure docs |
| 2 — S3 drift detector                                    | **Pending** | —               | —         | Skeleton already live (`3089057`); 13 seed signals (added `mc_whatsapp_disconnects_total`)                                                                                      |
| 3 — S5 skills-as-stored-procedures (all 5 phases per Q1) | **Pending** | —               | —         | Builds on v7.6 Spine 5 `src/skills/catalog.ts`; 5 production skills required                                                                                                    |
| 4 — Conway Pattern 1: general-events middle layer        | **Pending** | —               | —         | V8.1 Phase 1 prerequisite                                                                                                                                                       |
| 5 — Conway Pattern 2 + ⚠️ operator-profile skeleton (Q3) | **Pending** | —               | —         | Cohort roll-up + schema-only `operator_profile` table; anti-mission compliance test enforced                                                                                    |
| 6 — Conway Pattern 3: named recall modes                 | **Pending** | —               | —         | Coherence vs correspondence tag + weekly correspondence-audit cron                                                                                                              |
| 7 — Closure ritual + continuity tooling (Q4)             | **Pending** | —               | —         | `mc close-phase` / `mc resume` / `mc audit-closure`; v7.7 closure itself uses them as acceptance test                                                                           |
...
| **Parallel worker coordinator** (`src/video/worker-pool.ts` — written, tested, CAP 4)                                                                                           | hyperframes #3         | **Done** (S1.1 — wired into `composer.ts` 2026-05-08). Per-scene clip ffmpeg children fan out via `runPool`. Promisified `execFile`, capped pool size, per-task error capture, abort-signal guard, +2 tests. |
...
| Skill gate markdown (SKILL.md + house-style.md + visual-styles.md)                                                                                                                                                                 | —              | **Done v7.4.3.6** (2026-05-08). Shipped as `src/video/html-skills.ts` (3 catalogs: skill gate / house style / visual styles + injector), wired into `video_html_compose` description. Discoverable surface via `HTML_COMPOSE_SKILLS` for v7.5 engine. |
...
| ~~BM25 reflection memory — per-agent banks, inject top-2 lessons from past P&L into next prompts (~220 LOC)~~ ✅ **Done 2026-05-08 (L6 / A6)** — `src/finance/reflection-memory.ts`. Hand-rolled Robertson-Walker BM25 (no `wink-bm25-text-search` dep per CLAUDE.md). `ReflectionBank` (situation/lesson/pnl/ts) + `addLesson` / `retrieveTop` / `formatLessonsBlock` / serialize-deserialize / `ReflectionRegistry` for per-agent isolation. Defensive IDF floor (audit S1), single-digit numerics preserved (W2), divide-by-zero guard (W1). 14 tests.                                                                                                                                                                                                                                                                                                            | F7 (M2+)     | `reference_trading_agents.md`          |
| ~~Adversarial critic pass — single bull/bear critic over Portfolio Manager draft + judge reconciliation (~150 LOC)~~ ✅ **Done 2026-05-08 (L6 / A7)** — `src/finance/adversarial-critic.ts`. `runAdversarialCritique({draft, context, infer, ...})` runs bull + bear in parallel via `Promise.allSettled` (audit C1 — preserves both arms' tokens on either failure), then judge once both settle. Hard `withTimeout` per call (default 30s), fail-open to draft on any error/timeout. Trust-model JSDoc on `context` (audit W4 — must be system-derived, prompt-injection latent on user input). 13 tests including parallelism timing + token-preservation.                                                                                                                                                                                                        | F7           | `reference_trading_agents.md`          |
| ~~Black-Litterman signal combiner — blend multi-agent signals as views into posterior beliefs (~150 LOC, mathjs)~~ ✅ **Done 2026-05-08 (L5 / A8)** — `src/finance/black-litterman.ts`. He-Litterman 1999 closed form: `posteriorμ = inv(M) · (inv(τ·Σ)·π + Pᵀ·inv(Ω)·Q)`, `M = inv(τ·Σ) + Pᵀ·inv(Ω)·P`, `posteriorΣ = Σ + inv(M)`. Public API: `blackLitterman(input)`, `blackLittermanFromSignals({assets, pi, Sigma, signals, tau?, epsilon?})` (one-hot pick matrix + Idzorek-mapping `Ω = diag(τ·σ²/confidence)` per audit W3/W4 fix), `equilibriumReturnsReverse(weights, Σ, δ)`. NO mathjs — implemented `src/finance/matrix.ts` (matMul, matInverse w/ partial pivoting, matTranspose, diagMatrix; ~150L pure TS) per CLAUDE.md no-deps invariant. Non-diagonal Ω falls back from invertDiagonal to full matInverse (audit W6). 49 tests across matrix + BL. | F7           | `reference_skfolio.md`                 |
| ~~HRP weight allocator — Lopez de Prado hierarchical risk parity, replaces heuristic confidence-weighted sum~~ ✅ **Done 2026-05-08 (L4 / A9)** — `src/finance/hrp.ts`. Pure-TS port: correlation matrix → López de Prado distance → single-linkage clustering → quasi-diagonal leaf order → recursive bisection by inverse-variance. Diagonal-only `clusterVariance` (skfolio-mode simplification; full slice-cov is a follow-up). Float overshoot caught + clamped at source (`alpha-linalg.correlation()`). 25 tests including hand-computed 4-leaf tie-break determinism. Library-only — F7 callers wire when ready.                                                                                                                                                                                                                                             | F7           | `reference_skfolio.md`                 |
| ~~Inverse-volatility + equal-weight baseline allocators — ship before HRP as fallbacks (~30 LOC)~~ ✅ **Done 2026-05-08 (L4 / A10)** — `src/finance/allocators.ts`. `equalWeight(N)`, `inverseVolatility(returns)` w/ median-variance substitution for flat assets + last-asset rounding-error absorb so weights sum to exactly 1. `varianceVector()` exported as shared helper for both this module and HRP. 12 tests.                                                                                                                                                                                                                                                                                                                                                                                                                                              | F7 (M1)      | `reference_skfolio.md`                 |

**v7.5 anchored (implement during skill evolution engine):**

| Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Source                          | Memory Reference                |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- | ------------------------------- |
| ~~Prometheus reflector: per-dimension critiques with evidence instead of pass/fail~~ ✅ **Done 2026-05-08 (L3 / A4)** — `ReflectionResult.dimensions?: DimensionalCritique[]` (5 axes: completion / correctness / evidence_quality / effort / domain_coverage). System prompt asks for the 5-element array; `sanitizeDimensions()` drops malformed entries + clamps scores; `lowestDimension()` picks the replan target. Dimensions auto-dropped when score-override pathway fires (audit W1 fix) so they never contradict the kept score. +8 tests.                                                                                | RationalRewards paper           | `reference_rational_rewards.md` |
| ~~Predictive consistency gate: rationale must predict outcome without seeing answer~~ ✅ **Done 2026-05-08 (L3 / A5)** — New `src/tuning/predictive-consistency.ts`. `runPredictiveCheck()` probes ≤3 cases per mutation, asking the LLM to predict pass/fail given ONLY the hypothesis + case message. Wired into `overnight-loop.ts` after eval but before keep/discard, opt-in via `TUNING_PREDICTIVE_CONSISTENCY=true`. Failed gate adds a new `'rejected'` status branch but does NOT bump `consecutiveRegressions` (audit W2 — predictive failure ≠ regression). Boundary 0.5 score treated as fail (audit W3). +12 tests.    | RationalRewards paper           | `reference_rational_rewards.md` |
| ~~Skill evaluation loop: draft→test with/without→grade→improve cycle~~ ✅ **Done 2026-05-08 (L3 / A3)** — New `src/tuning/skill-eval-loop.ts`. `runSkillEval(skill, baselineSys, cases, inferFn)` runs each case twice (baseline / with-skill), grades against `{text, evidence}` assertions, returns `SkillEvalReport` with per-case deltas + `recommendation: adopt/refine/discard/reject`. Heuristic downgrades adopt → refine on n<3 samples. `emptyCaseIds` surfaced for fixture-gap visibility (audit W4). NOT wired into cron — operator activates manually until telemetry validates. +15 tests.                            | anthropics/skills skill-creator | `reference_anthropic_skills.md` |
| ~~Tool annotations: readOnlyHint, destructiveHint, idempotentHint on tool registry~~ ✅ **Done 2026-05-08 (L2)** — 4 hints (`readOnlyHint`/`destructiveHint`/`idempotentHint`/`openWorldHint`) on `Tool` interface + `getToolAnnotations()` normalizer (collapses absent hints to conservative MCP defaults; surfaces `requiresConfirmation`/`riskTier` in same view). 20 high-impact tools annotated (file/jarvis-fs/shell/web/code-search/http/gmail). 23 invariant tests including `readOnly⇒NOT destructive`, `requiresConfirmation⇒NOT readOnly`, `riskTier:high⇒destructive`. Convention documented in CLAUDE.md ACI section. | anthropics/skills mcp-builder   | `reference_anthropic_skills.md` |
