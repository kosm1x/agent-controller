...
| `EMAIL_<ID>_PERSONA_FILE`     | —                   | —            | Path to a markdown file with org mission/voice/contacts/reply rules. Recommended in `community-manager` mode. Loaded once at startup (cap 1 MiB). Relative paths resolve from the repo root. See `docs/EMAIL-PERSONA-EXAMPLE.md`. |
