...
- **Jev shadow consumers (2026-09-22 `6cf00fe`)**: `JEV_SHADOW_CONSUMERS` (mission-control env; comma list, valid values `kb`, `feedback`; default empty = dormant) arms a consumer that logs Jev's answer beside the live decision into `jev_shadow` (migration v6) and changes nothing: `kb` scores the 7 conditional KB rows registered in `KB_SHADOW_ROWS` (`src/jev/shadow-kb.ts`) by their committed English descriptions — **no KB text ever leaves the box**, an unregistered row is not sent; `feedback` asks correction / restatement about a follow-up inside the feedback window (no positive question — "excelente" is the only eval word). `memory` is a CHECK value with no consumer behind it (deferred, operator ruling A). Every shadow call is `setImmediate` + never awaited. Rule for any new consumer: the filter reads the whole source text before anything is cut; only `ask()` in `src/jev/shadow.ts` cuts, after filtering. Before arming `kb`, read what would leave: `npx tsx scripts/validate-jev-kb.ts --show`. Readout bars and the arm order (`kb` → `feedback`) are in `docs/planning/jev-consumers-plan-2026-09-21.md`; readout = `SELECT consumer, item, count(*) FROM jev_shadow GROUP BY 1, 2` after 7 days and ≥ 150 decisions.
