...
- **Scope classifier provider (2026-09-21 `3c33771`, operator ruling)**: TypeSafe Jev classifies first (`src/messaging/scope-classifier-jev.ts`, threshold 0.70, 1.5 s deadline) whenever `TYPESAFE_API_KEY` is present; any non-answer — no key, non-2xx, malformed body, timeout, withheld text — falls through to the Sonnet classifier below, then the regex fallback. Kill switch: `SCOPE_CLASSIFIER_PROVIDER=sonnet` + restart. Measured offline at 89.5 % held-out coverage on Spanish turns (plan `docs/planning/jev-decision-layer-plan-2026-09-21.md` §5; the plan's 95 % bar FAILED, the operator accepted the number). Leaves the box every turn: the message, ≤ 150 chars of the previous thread turn, and the classifier prompt's rules/examples/group descriptions; credential-shaped text, bare opaque tokens and e-mail addresses never do (turn goes to Sonnet). Evidence of which classifier answered = journal only (`[scope-classifier] jev: [...] in N ms` / `jev failed … → sonnet` / `jev skipped`); `scope_telemetry` has no provider column yet. Do not retune `JEV_SCOPE_THRESHOLD` from live impressions — it is the registered value; a change needs a new registered replay.
